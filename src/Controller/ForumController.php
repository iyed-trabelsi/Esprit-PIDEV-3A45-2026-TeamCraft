...
class ForumController extends AbstractController
{
    ...
    public function commentEdit(Request $request, int $id): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // Validation Manuelle (PHP) - le User veut que ça soit contrôlé ici
            if (empty($contenu)) {
                $form->get('contenu')->addError(new \Symfony\Component\Form\FormError('Le commentaire ne peut pas être vide.'));
                 ...
                 $form->get('contenu')->addError(new \Symfony\Component\Form\FormError('Le commentaire est trop court (min 3 caractères).'));
            }
            ...
            // Si erreurs ajoutées manuellement, le formulaire est invalide donc on réaffiche
