...
class TeamController extends AbstractController
{
    ...
    public function manage(int $id, EntityManagerInterface $em, Request $request, ValidatorInterface $validator): Response
    {
        ...
        if ($offerForm->isSubmitted() && $offerForm->isValid()) {
            // Get validity period from form (unmapped field)
            $validityPeriod = $offerForm->get('validityPeriod')->getData();
            
            // Calculate expiration date
            $creationDate = new \DateTime();
            $expirationDate = (clone $creationDate)->modify("+{$validityPeriod} days");
