...
class CheckOfferExpirationCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($activeOffers as $offer) {
            $dateExpiration = $offer->getDateExpiration();
            
            // Auto-expire if expiration date is reached
            if ($dateExpiration && $dateExpiration <= $now) {
                $offer->setStatus(Offer::STATUS_EXPIRED);
                $this->notificationService->createNotification(
                    $offer->getTeam()->getOwner(),
                    'OFFER_EXPIRED',
                    sprintf('Your offer "%s" has expired.', $offer->getTitle())
                );
